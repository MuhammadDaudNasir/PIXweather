              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <AnimatePresence>
                  {favorites.map((location, index) => (
                    ...
                    >
                      <Card className="overflow-hidden border-0 bg-black/40 backdrop-blur-xl border border-[#3c2a21]/30 h-full">
                        ...
                        <CardContent className="p-4">
                          <p className="text-sm text-[#d5bdaf]/80 mb-4 line-clamp-2">{location.description}</p>
                          <div className="flex justify-between items-center">
                            <Link href={`/?location=${encodeURIComponent(location.name)}`}>
                              <Button className="bg-[#8B4513] hover:bg-[#6d3710] text-white">View Location</Button>
                            </Link>
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => removeFromFavorites(location.name)}
                              className="rounded-full bg-black/40 backdrop-blur-md border border-[#3c2a21]/30 text-[#d5bdaf] hover:bg-[#3c2a21]/50 h-9 w-9"
                            >
